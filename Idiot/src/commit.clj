...
(defn update_ref_refer [working_directory db head_referred address]
  (spit (io/file (str working_directory "/" db "/" "refs/heads/" head_referred)) (str address "\n")))

(defn update_ref [working_directory db address]
  (cond (not (= 41 (count (slurp (io/file (str working_directory "/" db "/HEAD"))))))
        (let [refs (set (sort (seq (.list (io/file (str working_directory "/" db "/refs/heads"))))))
              head_referred (str/trim-newline (subs (slurp (io/file (str working_directory "/" db "/HEAD"))) 16))]
          (println (str "Updated branch " head_referred "."))
          (cond (contains? refs head_referred) (update_ref_refer working_directory db head_referred address)
                :else (spit (io/file (str working_directory "/" db "/" "refs/heads/" head_referred)) (str address "\n"))))))

(defn process_parent_info [working_directory db tree_address message parent_info]
  (if (empty? parent_info)
    (update_ref working_directory db (build_commit_object_0 working_directory db tree_address message))
    (let [p (first parent_info)
      ...
      (cond (and (= 0 (compare p "-p")) (empty? parent_address)) (println "Error: you must specify a commit object with the -p switch.")
            ...
            :else (cond (= length 2) (update_ref working_directory db (build_commit_object_1 working_directory db tree_address message parent_info))
                        (= length 4) (update_ref working_directory db (build_commit_object_2 working_directory db tree_address message parent_info))
                        (= length 16) (update_ref working_directory db (build_commit_object_8 working_directory db tree_address message parent_info)))))))

(defn commit [working_directory db content]
  (let [tree_address (first content)
        left (rest content)
    ...
    (cond (or (= 0 (compare tree_address "--help")) (= 0 (compare tree_address "-h"))) (help-instructions-commit)
          ...
          (= 0 (compare "noneexist" (if-repeated working_directory db tree_address))) (let [output "Error: no tree object exists at that address."]
                                                                                        (println output))
